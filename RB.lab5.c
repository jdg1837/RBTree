...
//checkN(head); N not properly found
}
...
void STdeserialize(char* input)
{
	int i = 0;
	head = preorder_in(input, &i);
}

link preorder_in(char* input, int* i)
{
	char buffer[10];
	char color;
	link node;
	...
	else
	{
		...
		*i = j + 1;
		...
		node->item = atoi(buffer);
		node->red = color;

		node->l = preorder_in(input, i);
		node->r = preorder_in(input, i);

		node->N = (node->l->N) + (node->r->N) + 2;
	}

	return node;
}

char* STserialize(char* output)
{
	int count = 0;

	output = preorder_out(head, output, &count);

	return output;
}

char* preorder_out(link node, char* output, int* count)
{
	char buffer[10];
	char* temp;
	char color;
	...
	else
	{
		...
		sprintf(buffer,"%d%c", node->item, color);
	}
	...
	*count += strlen(buffer);
    ...
    preorder_out(node->l, output, count);  
 
    preorder_out(node->r, output, count);
